...
+ (void)initialize
{
    checkedIcon     = [UIImage imageWithContentsOfFile:[[[NSBundle mainBundle] resourcePath] stringByAppendingPathComponent:[NSString stringWithFormat:@"VideoPicker.Bundle/Images/%@@2x.png",(!IS_IOS7) ? @"AssetsPickerChecked~iOS6" : @"AssetsPickerChecked"]]];
...
@property (nonatomic, retain) VideoTitleView *videoTitle;
@property (nonatomic, retain) TapAssetView *tapAssetView;
...
@interface AssetViewCell ()<AssetViewDelegate>
    ...
    if (self.contentView.subviews.count<assets.count) {
        for (int i=0; i<assets.count; i++) {
            if (i>((NSInteger)self.contentView.subviews.count-1)) {
                ...
                [assetView bind:assets[i] selectionFilter:selectionFilter isSeleced:[((AssetViewController*)_delegate).indexPathsForSelectedItems containsObject:assets[i]]];
                ...
                [(AssetView*)self.contentView.subviews[i] bind:assets[i] selectionFilter:selectionFilter isSeleced:[((AssetViewController*)_delegate).indexPathsForSelectedItems containsObject:assets[i]]];
            }
        ...
        for (NSInteger i=self.contentView.subviews.count; i>0; i--) {
            if (i>assets.count) {
                ...
                [(AssetView*)self.contentView.subviews[i-1] bind:assets[i-1] selectionFilter:selectionFilter isSeleced:[((AssetViewController*)_delegate).indexPathsForSelectedItems containsObject:assets[i-1]]];
            }
        }
    }
}
...
@interface AssetViewController ()<AssetViewCellDelegate>{
    int columns;
...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    VideoPickerController *picker = (VideoPickerController *)self.navigationController;
...
#pragma mark - AssetViewCell Delegate

- (BOOL)shouldSelectAsset:(ALAsset *)asset
{
    
    
    VideoPickerController *vc = (VideoPickerController *)self.navigationController;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    AssetGroupViewCell *cell = [tableView dequeueReusableCellWithIdentifier:CellIdentifier];
    if (cell == nil)
    {
        cell = [[AssetGroupViewCell alloc] initWithStyle:UITableViewCellStyleSubtitle reuseIdentifier:CellIdentifier];
    }
