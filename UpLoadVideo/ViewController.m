...
@interface ViewController () <VideoPickerControllerDelegate, UINavigationControllerDelegate, UpLoadImageDeletate>
...
- (void)viewDidLoad {
    ...
    videoImage.frame=CGRectMake(36, titLabel.bottom+10, SCREENWIDTH-36*2,160);
    videoImage.backgroundColor=[UIColor lightGrayColor];
    [self.view addSubview:videoImage];
    _videoImage=videoImage;
    
    UIButton *chooseBtn=[[UIButton alloc]init];
    chooseBtn.frame=CGRectMake(47,videoImage.bottom+10 ,SCREENWIDTH-47*2 , 30);
    [chooseBtn setTitle:@"选择视频" forState:UIControlStateNormal];
    ...
    uploadBtn.frame=CGRectMake(47,chooseBtn.bottom+15 ,SCREENWIDTH-47*2 , 30);
...
- (void)upLoadVideo {
    VideoPickerController *picker = [[VideoPickerController alloc] init];
...
- (void)compressionVideoWithURL:(NSURL *)url videoMemorySize:(NSInteger)videoMemerySize upLoadMeneger:(UpLoadImage *)upLoadMeneger {
    ...
    __weak typeof(self)weakSelf = self;
    // 调用压缩类, 进行压缩
    [CompressionVideo compressedVideoOtherMethodWithURL:url compressionType:AVAssetExportPresetMediumQuality compressionResultPath:^(NSString *resultPath, float memorySize) {
...
-(void)cleanAllVideoData {
    ...
    [CompressionVideo removeCompressedVideoFromDocuments];
}
